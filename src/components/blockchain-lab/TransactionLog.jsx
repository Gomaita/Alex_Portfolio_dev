  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        ...
        <p className="mt-2 text-sm text-slate-400">A mini explorer for simulated contract events. These are not real transactions.</p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <select value={eventType} onChange={(event) => setEventType(event.target.value)} className="min-h-11 rounded-lg border border-slate-700 bg-slate-950 px-3 text-white">
            <option value="all">All events</option>
            {eventTypes.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <select value={role} onChange={(event) => setRole(event.target.value)} className="min-h-11 rounded-lg border border-slate-700 bg-slate-950 px-3 text-white">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
        <div className="space-y-3">
          {filtered.map((tx) => (
            <button key={tx.id} type="button" onClick={() => setSelectedId(tx.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selected?.id === tx.id ? 'border-violet-400 bg-violet-500/10' : 'border-slate-800 bg-slate-900 hover:border-slate-700'}`}>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          {selected ? (
            <>
              ...
              <p className="mt-2 text-sm text-slate-400">Simulated transaction</p>
              ...
              <button type="button" onClick={() => copyHash(selected.fakeTxHash)} className="mt-5 inline-flex max-w-full items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 text-sm font-semibold text-slate-200 hover:border-violet-400">
                <Copy size={15} />
