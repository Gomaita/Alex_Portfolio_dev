      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">Review simulated incidents derived from firewall events.</p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <select value={status} onChange={(event) => setStatus(event.target.value)} className="min-h-11 rounded-lg border border-slate-700 bg-slate-950 px-3 text-sm text-white">
        ...
        <div className="space-y-3">
          {filteredIncidents.map((incident) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-400">{incident.recommendation}</p>
            </button>
          ))}
        </div>
            ...
            <>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-400">{selectedIncident.source} - {selectedIncident.createdAt}</p>
                </div>
